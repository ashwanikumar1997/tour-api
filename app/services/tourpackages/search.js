 ...
 */
function searchTourPackage(keyword, limit, columns, user) {
    ...
    if (user) {
        if (!user._isAdmin) {
            findQuery['$or'] = [
                ...
                {reviewStatus: TourPackageReviewStatus.approved}
            ];
        }
    }
    else {
        findQuery.reviewStatus = TourPackageReviewStatus.approved;
    }
...
module.exports = searchTourPackage
