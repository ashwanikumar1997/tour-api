 ...
 */
function searchBookings(keyword, limit, columns, user) {
    ...
    if (user) {
        if (!user._isAdmin) {
            findQuery['$or'] = [
                ...
                { reviewStatus: BookingReviewStatus.approved }
            ];
        }
    }
    else {
        findQuery.reviewStatus = BookingReviewStatus.approved;
    }
     ...
     */
    bookingpackageQuery = Booking.find(findQuery).limit(limit).lean();
    //}


    return bookingpackageQuery.exec();
}
